...
- Name: producto.spec.ts >> Login - demoblaze >> Login exitoso con credenciales válidas
- Location: tests\producto.spec.ts:10:9

# Error details

```
Error: locator.innerText: Error: strict mode violation: locator('a[href="prod.html?idp_=1"]') resolved to 2 elements:
...
```ts
  ...
  2  | 
  3  | test.describe('Login - demoblaze', ()=> {
  4  | 
  ...
  8  |     });
  9  | 
  10 |     test('Login exitoso con credenciales válidas', async ({ page }) => {
  11 |         //Guardar en un variable el elemento
  12 |         const linkProducto = page.locator('a[href="prod.html?idp_=1"]');
  13 |         //Extraemos el texto para validar despues
> 14 |         const nombreProducto = await linkProducto.innerText();
     |                                                   ^ Error: locator.innerText: Error: strict mode violation: locator('a[href="prod.html?idp_=1"]') resolved to 2 elements:
  15 |         //CLick en elemento
  16 |         linkProducto.click();
  17 |         //await page.locator('a[href="prod.html?idp_=1"]').click();
  18 |         //Validar titulo del producto
  19 |         const titulo = page.locator('//h2[@class="name"]');
  20 |         expect(titulo).toHaveText(nombreProducto);
  21 |         //Esperar boton add
  22 |         await await page.getByRole('link', { name: 'Add to cart' }).waitFor({ state: 'visible' });
  23 | 
  24 | 
  25 | 
  26 | 
